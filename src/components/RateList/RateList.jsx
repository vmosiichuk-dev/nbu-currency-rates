...
export const RateList = ({ customRates, hideExtraColumns = false }) => {
	...
	const { rates, ratesPending, topRates, ratesDiffer, todayDate } = useDateSearchParams();
	...
	const ratesToMap = customRatesArray || (hideExtraColumns ? topRates : rates) || [];
	...
	const missingRowsCount = Math
		.max(0, rowsPerPage - (ratesToMap?.length || 0));

	const emptyHeight = (rowHeight + 6) * missingRowsCount;

	const gridAreas = hideExtraColumns || isMediaSM
		? `'currency rate'`
